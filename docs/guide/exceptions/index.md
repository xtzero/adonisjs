...
In this guide we will go through the list of known exceptions raised by the framework core and the official packages. Some of the exceptions are marked as **self-handled**. [Self-handled exceptions](/guide/exception_handling/index.html#defining-the-handle-method) can convert themselves to an HTTP response.

<div style="--prose-h2-font-size: 22px;">

## E_ROUTE_NOT_FOUND
The exception is raised when the HTTP server receives a request for a non-existing route. By default, the client will get a 404 response, and optionally, you may render an HTML page using [status pages](/guide/exception_handling/index.html#status-pages).
...
The exception is raised when a bouncer authorization check fails. The exception is self-handled and [uses content-negotiation](/guide/authorization/index.html#throwing-authorizationexception) to return an appropriate error response to the client.
...
## E_TOO_MANY_REQUESTS
The exception is raised by the [@adonisjs/rate-limiter](/guide/rate-limiter/) package when a request exhausts all the requests allowed during a given duration. The exception is self-handled and [uses content-negotiation](/guide/rate-limiter/index.html#handling-throttleexception) to return an appropriate error response to the client.
...
## E_BAD_CSRF_TOKEN
The exception is raised when a form using [CSRF protection](/guide/web-security/index.html#csrf-protection) is submitted without the CSRF token, or the CSRF token is invalid.
...
You can avoid this exception if you [handle the errors](/guide/social-authentication/index.html#handling-callback-response) before calling the `.accessToken` or `.user` methods.
...
The exception is raised when one of the authentication guards is not able to authenticate the request. The exception is self-handled and uses [content-negotiation](/guide/session_guard/index.html#handling-authentication-exception) to return an appropriate error response to the client.
...
The exception is raised when the auth finder is not able to verify the user credentials. The exception is handled and use [content-negotiation](/guide/verifying_user_credentials/index.html#handling-exceptions)  to return an appropriate error response to the client.
...
The exception is raised when you attempt to create a URL for a route using the [URL builder](/guide/url_builder/).
...
The `E_HTTP_REQUEST_ABORTED` is a sub-class of the `E_HTTP_EXCEPTION` exception. This exception is raised by the [response.abort](/guide/response/index.html#aborting-request-with-an-error) method.
...
## E_INSECURE_APP_KEY
The exception is raised when the length of `appKey` is smaller than 16 characters. You can use the [generate:key](/guide/commands/index.html#generatekey) ace command to generate a secure app key.
...
The `@adonisjs/redis` package raises the exception when you attempt to [subscribe to a given pub/sub channel](/guide/redis/index.html#pubsub) multiple times.
...
The `@adonisjs/redis` package raises the exception when you attempt to [subscribe to a given pub/sub pattern](/guide/redis/index.html#pubsub) multiple times.
