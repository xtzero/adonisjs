...
## request
Reference to the instance of ongoing [HTTP request](/guide/request/). The property is only available when a template is rendered using the `ctx.view.render` method.
...
## route/signedRoute {#routesignedroute}
Helper functions to create URL for a route using the [URL builder](/guide/url_builder/index.html#generating-urls-inside-templates). Unlike the URL builder, the view helpers do not have a fluent API and accept the following parameters.
...
## app
Reference to the [Application instance](/guide/application/).
...
## config
A [helper function](/guide/config/index.html#reading-config-inside-edge-templates) to reference configuration values inside Edge templates. You may use the `config.has` method to check if the value for a key exists.
...
## session
A read-only copy of the [session object](/guide/session/index.html#reading-and-writing-data). You cannot mutate session data within Edge templates. The `session` property is only available when the template is rendered using the `ctx.view.render` method.
...
## flashMessages
A read-only copy of [session flash messages](/guide/session/index.html#flash-messages). The `flashMessages` property is only available when the template is rendered using the `ctx.view.render` method.
...
## t
The `t` method is contributed by the `@adonisjs/i18n` package to display translations using the [i18n class](/guide/internationalization/index.html#resolving-translations). The method accepts the translation key identifier, message data and a fallback message as the parameters.
...
## i18n
Reference to an instance of the I18n class configured using the application's default locale. However, the [`DetectUserLocaleMiddleware`](/guide/internationalization/index.html#detecting-user-locale-during-an-http-request) overrides this property with an instance created for the current HTTP request locale.
...
## auth
Reference to the [ctx.auth](/guide/http_context/index.html#http-context-properties) property shared by the [InitializeAuthMiddleware](https://github.com/adonisjs/auth/blob/main/src/auth/middleware/initialize_auth_middleware.ts#L14). You may use this property to access information about the logged-in user.
...
## asset
Resolve the URL of an asset processed by Vite. Learn more about [referencing assets inside Edge templates](/guide/assets_bundling/index.html#referencing-assets-inside-edge-templates).
...
The `embedImage` and the `embedImageData` helpers are added by the [mail](../guide/configuring-message/index.html#embedding-images) package and are only available when rendering a template to send an email.
...
See also: [Pre-registering abilities and policies](/guide/authorization/index.html#pre-registering-abilities-and-policies)
